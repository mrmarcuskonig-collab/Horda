...
const r = await fetch(b+'/events',{method:'POST',redirect:'manual',headers:{'content-type':'application/x-www-form-urlencoded'},
  body:new URLSearchParams({host_kind:'athlete',host_id:h,title:'Fight Night · Kreuzberg',starts_at:soon,location_kind:'hybrid',location:'Kreuzberg Boxing Club',
...
fs.writeFileSync('/tmp/w_create.html', await (await fetch(b+`/host/athlete/${h}/new`)).text());
